	...
	$.ajax({
		...
		"success":function(response){
			...
			if(result == "SUCCESS") {
				...
				//zTreeObj.expandAll(false);//关闭所有的节点
			}
...
function myAddHoverDom(treeId, treeNode) {
	...
	// 判断一下以前是否已经添加了按钮组[之所以需要这样一步操作是因为移入鼠标可能会多次处罚，因为移入鼠标有一段时间的持续]
...
function myAddDiyDom(treeId, treeNode) {
	...
	/*图标生成是默认的，他有自己的生成规则，我们要改的话就要改它里面的Class属性来修改图标样式：
	* eg:原来描述这个标签的<span>结构是下面这样：
	* <span id="treeDemo_7_ico" title="" treenode_ico="" class="button ico_close" style="background:url(glyphicon glyphicon-ok) 0 0 no-repeat;"></span>
	* 其中的class就是描述图标的
	* */
	// zTree生成id的规则
	// 例子：treeDemo_7_ico
